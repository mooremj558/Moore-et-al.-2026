...
for i = 1:chrom
    ...
    if split(split(:,1)==i,2)~=0
        ...
        pos_part = [0,mid,rows];
        turn = 2;
        %adding begain, to split arms
        if i == 1
            split1 = ceil(mid/2);
            split2 = ceil(mid+1*(rows-mid)/4);
            split3 = ceil(mid+2*(rows-mid)/4);
            split4 = ceil(mid+3*(rows-mid)/4);
            pos_part = [0,split1,mid,split2,split3,split4,rows];
            turn = 6;
        end
        if i == 2
        ...
        if i == 3
            split1 = ceil(mid/2);
            split2 = ceil(mid+1*(rows-mid)/2);
            pos_part = [0,split1,mid,split2,rows];
            turn = 4;          
        end
        if i == 4
        ...
        if i == 23
            split1 = ceil(mid+1*(rows-mid)/6);
            split2 = ceil(mid+2*(rows-mid)/6);
            split3 = ceil(mid+3*(rows-mid)/6);
            split4 = ceil(mid+4*(rows-mid)/6);
            split5 = ceil(mid+5*(rows-mid)/6);
            pos_part = [0,mid,split1,split2,split3,split4,split5,rows]
            turn = 7;          
        end   
        %adding end
    else
