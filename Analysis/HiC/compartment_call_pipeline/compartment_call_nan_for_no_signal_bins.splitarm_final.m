...
for i = 1:chrom
    ...
    if split(split(:,1)==i,2)~=0
        ...
        if i == 1
            split1 = ceil(1*mid/3);
            split2 = ceil(2*mid/3);
            split3 = ceil(mid+2*(rows-mid)/4);
            split4 = ceil(mid+3*(rows-mid)/4);
            pos_part = [0,split1,split2,mid,split3,split4,rows];
            turn = 6;
        end
        if i == 2
            split1 = ceil(mid+1*(rows-mid)/4);
            split2 = ceil(mid+2*(rows-mid)/4);
            split3 = ceil(mid+3*(rows-mid)/4);
            pos_part = [0,mid,split1,split2,split3,rows];
            turn = 5;
        end
        if i == 3
            split1 = ceil(1*mid/2);
            split2 = ceil(mid+1*(rows-mid)/3);
            split3 = ceil(mid+2*(rows-mid)/3);
            pos_part = [0,split1,mid,split2,split3,rows];
            turn = 5;
        end
        if i == 4
            split1 = ceil(mid+1*(rows-mid)/3);
            split2 = ceil(mid+2*(rows-mid)/3);
            pos_part = [0,mid,split1,split2,rows];
            turn = 4;
        end
        if i == 5
            split1 = ceil(mid+1*(rows-mid)/3);
        ...
        if i == 6
            split1 = ceil(mid+1*(rows-mid)/2);
            pos_part = [0,mid,split1,rows];
            turn = 3;
        end
        if i == 7
            split1 = ceil(mid+1*(rows-mid)/2);
        ...
        %end
        if i == 23
            split1 = ceil(mid+1*(rows-mid)/5);
            split2 = ceil(mid+2*(rows-mid)/5);
            split3 = ceil(mid+3*(rows-mid)/5);
            split4 = ceil(mid+4*(rows-mid)/5);
            pos_part = [0,mid,split1,split2,split3,split4,rows]
            turn = 6;
        end   
        %adding end
    else
        ...
        if i == 13
            split1 = ceil(rows/2);
            pos_part = [0,split1,rows];
            turn = 2;
        end
    end
    info_compart = [];
    for part = 1:turn
        fprintf('Part%d is going to be processed!\n',part);
