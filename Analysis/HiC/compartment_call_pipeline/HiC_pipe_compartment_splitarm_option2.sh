...
# usage: bash HiC_pipe_compartment_splitarm_final.sh -j -h -m -b -bn -n
#####################################################################
...
MAXJOBS=1 #Cores per CPU

HIC_PATH="/scratch/yliang/Epitherapy_3D/data/hi-C/BRep" # path to hic files
HIC_FILE_EXTENSION=".hic"
MATRIX_PATH="/scratch/yliang/Epitherapy_3D/data/hi-C/BRep_juicer_50k_KR_observed_matrix" # path to output matrix files
...
## Compartment call
find ${HIC_PATH} -maxdepth 1 -name "*${HIC_FILE_EXTENSION}" | while read file ; do xbase=$(basename $file ${HIC_FILE_EXTENSION}) ; echo "bash ${PIPE_PATH}/2_mat2compartment_splitarm_option2.sh --basename ${xbase} --matrix_path ${MATRIX_PATH} --output_path $PWD --bin_size ${BIN_SIZE} --bin_size_n ${BIN_SIZE_n} --normalization ${NORMALIZATION} --pipe_path ${PIPE_PATH}" >> 2_mat2compartment_commands.txt ; done ;
