...
parser.add_argument("--split", help = "specify how many splitting is happening in this arm", type = str)
parser.add_argument("--part", help = "specify which region needs to be flipped", type = str)
parser.add_argument("--splitfile", help = "path to chromosome split file", default = "/bar/yliang/genomes/private/hg38_chrom_split.txt", type = str)
parser.add_argument("--chrsizefile", help = "path to chromosome size file", default = "/bar/yliang/genomes/private/hg38_23_chromosomes.size", type = str)
...
part_input = args.part
split_file = args.splitfile
size_file = args.chrsizefile
...
split["chrX"] = split["chr23"]

with open(size_file, 'r') as file:
	for line in file:
		entry = line.strip('\n').split('\t')
		size[entry[0]] = int(entry[1])

chrom_nums = chrom_nums_input.split(',')

chrom_arms = chrom_arms_input.split(',')

split_nums = [int(x) for x in split_input.split(',')]

part_nums = [int(x) for x in part_input.split(',')]
...
###############################################################
for chr_num_num, chr_arm, split_num, part_num in zip(chrom_nums, chrom_arms, split_nums, part_nums):
	chr_num = "chr" + str(chr_num_num)
	chr_split = split[chr_num]
	chr_size = size[chr_num]
	for flag in eigen:
		if eigen[flag][-1][0] != chr_num:
			eigen[flag].append(eigen[flag][-1])
			continue
		if eigen[flag][-1][0] == chr_num:
			if chr_arm == "p" and chr_split/split_num*(part_num-1) <= int(eigen[flag][-1][2]) <= chr_split/split_num*part_num:
				eigen[flag].append([eigen[flag][-1][0], eigen[flag][-1][1], eigen[flag][-1][2], float(eigen[flag][-1][3])*-1])
				continue
			if chr_arm == "q" and chr_split+(chr_size-chr_split)/split_num*(part_num-1) <= int(eigen[flag][-1][1]) <= chr_split+(chr_size-chr_split)/split_num*part_num:
				eigen[flag].append([eigen[flag][-1][0], eigen[flag][-1][1], eigen[flag][-1][2], float(eigen[flag][-1][3])*-1])
				continue
			else:
				eigen[flag].append(eigen[flag][-1])
